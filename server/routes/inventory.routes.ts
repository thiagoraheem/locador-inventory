...
export async function registerInventoryRoutes(app: Express) {
  ...
  app.get(
    "/api/inventories/:id/dashboard-snapshot",
    isAuthenticated,
    async (req: any, res) => {
      try {
        storage = await getStorage();
        ...
        const rawItems = await storage.getInventoryItemsWithDetails(inventoryId);
        ...
        const dashboardItems = (rawItems || []).map((item: any, index: number) => {
          ...
          return {
            itemId: `${inventory.code}-${String(index + 1).padStart(6, "0")}`,
        ...
        const movementsBlocked = Boolean(inventory?.isToBlockSystem);
        ...
        res.json({
          ...
          compliance: {
            ...
            blockSystemMovements: movementsBlocked,
            signedLists: false,
        ...
        res.status(500).json({
          message: "Failed to fetch inventory dashboard snapshot",
          details: (error as Error).message,
        });
      }
    },
